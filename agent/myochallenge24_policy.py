...
from jax.lax import stop_gradient, associative_scan
...
class dynamics(nn.Module):
    ...
    def setup(self):
        ...
        dynamics = [nn.Sequential([nn.Dense(features=h_dim), nn.LayerNorm(), nn.relu]) for h_dim in self.h_dims_dynamics]
        dynamics.append(nn.Dense(features=len(self.control_variables)*2))
        self.dynamics = dynamics

        self.dropout = nn.Dropout(rate=self.drop_out_rate)

        self.inverse_softplus_sigma = self.param("inverse_softplus_sigma", init_fn=lambda key: jnp.full((), jnp.log(jnp.exp(1.)-1.)))
    ...
    def __call__(self, obs, u, key, deterministic):
        ...
        # mask task-specific variables
        obs = obs.at[...,self.task_variables].set(0.)

        x = jnp.concatenate((obs[...,self.obs_variables], u[...,self.action_variables_dynamics]), axis=-1) # state-dependent dynamics/synergies
        # x = jnp.copy(u) # state-independent dynamics/synergies
...
def posterior_inference(obs, next_obs, synergies_state, dynamics_state, key, control_variables):
    ...
    def get_posterior(residual_power, Jac, A, mu_u_source, E_y_prime, s_prime_var, next_obs, control_variables):

        z_dim = len(control_variables)

        # prior p(z|x) = N(mu_z, Lambda^-1) - the optimal source
        mu_z = jnp.zeros(z_dim)
        Lambda = 1. / (residual_power / z_dim) * jnp.eye(z_dim) # equal power allocation assumed here
        ...
        # posterior p(z|y',x) = N(mu_posterior, Sigma_posterior)
        Sigma_posterior = psd_solve(Lambda + A_eff.T @ L @ A_eff, np.eye(z_dim)) # jnp.linalg.inv(Lambda + A_eff.T @ L @ A_eff)
        mu_posterior = Sigma_posterior @ (A_eff.T @ L @ (next_obs[...,control_variables] - b_eff) + Lambda @ mu_z)

        return mu_posterior, Sigma_posterior

    batch_get_posterior = jax.vmap(get_posterior, in_axes=(0,0,0,0,0,0,0,None))

    _, _, _, mu_u_source, A, _, _, _, residual_power, Jac, E_y_prime, s_prime_var = synergies_state.apply_fn(synergies_state.params, obs, dynamics_state, key)

    mu_posterior, Sigma_posterior = batch_get_posterior(residual_power, Jac, A, mu_u_source, E_y_prime, s_prime_var, next_obs, control_variables)
...
class synergies(nn.Module):
    ...
    def setup(self):
        ...
        source = [nn.Sequential([nn.Dense(features=h_dim), nn.relu]) for h_dim in self.h_dims_source]
        source.append(nn.Dense(features=self.action_dim+1))
        self.source = source

        self.standard_basis = np.eye(self.z_dim,self.action_dim)

    def __call__(self, obs, dynamics_state, key, deterministic, cocontraction, squash_controls_fn):
        ...
        def modified_gram_schmidt(vectors, init_i=0):
            ...
            def orthogonalise_wrt_subspace(subspace_vecs, vec):
                ...
                return vec
            batch_orthogonalise_wrt_subspace = jax.vmap(orthogonalise_wrt_subspace, in_axes=(None,1), out_axes=(1))
            def body_fn(vecs, i):
            ...
            if init_i != 0:
                null_space_basis = batch_orthogonalise_wrt_subspace(vectors[:,:init_i], vectors[:,init_i:]) # better to batch than scan where possible (synergies already orthonormalised)
                vectors = jnp.concatenate((vectors[:,:init_i], null_space_basis),axis=1)
            vectors, _ = jax.lax.scan(body_fn, vectors, jnp.arange(init_i, num_vectors - 1))
        ...
        def dynamics_forward_pass(u, dynamics_state, obs, deterministic, key, squash_controls_fn):
            ...
            u = squash_controls_fn(jnp.concatenate((u,jnp.zeros(17)), axis=-1)) #  add dummy, it gets ignored in dynamics anyway

            y_prime_mean, s_prime_log_var = dynamics_state.apply_fn(dynamics_state.params, obs, u, key, deterministic)

            return y_prime_mean, (y_prime_mean, jnp.exp(s_prime_log_var))

        batch_get_jacobian = jax.vmap(jax.jacrev(dynamics_forward_pass, has_aux=True), in_axes=(None,None,None,None,0,None))
        ...
        def get_orthogonal_bases(A, key):
            ...
            A_augmented = jnp.concatenate((A, jax.random.normal(key, (63, 63-self.z_dim))), axis=1)
            
            # Q, _ = jnp.linalg.qr(A_augmented)
            # # A_perp = jnp.concatenate((A, Q[:,self.z_dim:]), axis=1)
            # A_perp = Q[:,self.z_dim:]

            A_perp = modified_gram_schmidt(A_augmented,init_i=self.z_dim)[:,self.z_dim:]

            return A_perp

        def get_synergies(mu_u_source, obs, dynamics_state, residual_power, key, deterministic, squash_controls_fn):
            ...
            (Jac, (y_prime_mean, s_prime_var)) = batch_get_jacobian(mu_u_source[...,:63], dynamics_state, obs, deterministic, subkeys, squash_controls_fn)
            ...
            Sigma_u_source = residual_power / self.z_dim * A @ jnp.eye(self.z_dim) @ A.T # equal power allocation assumed here
            ...
            return A, explore_var, A_complement, 0., 0., Jac, y_prime_mean, s_prime_var

        batch_get_synergies = jax.vmap(get_synergies, in_axes=(0,0,None,0,0,None,None))
        ...
        if self.cocontraction is not None:
            mu_u_source = jnp.ones((obs.shape[0], self.action_dim))*nn.sigmoid(cocontraction)*-1.
        else:
            if len(self.source_mu) == 1:
                mu_u_source = jnp.ones((obs.shape[0], self.action_dim))*self.source_mu[0] # -0.5 and -0.4 decent
            else:
                mu_u_source = jnp.array(self.source_mu)[None,:].repeat(obs.shape[0],axis=0)
        ...
        A, explore_var, A_complement, p_ratio_source, entropy_marginal_transition, Jac, y_prime_mean, s_prime_var = batch_get_synergies(mu_u_source, obs, dynamics_state, residual_power, keys, deterministic, squash_controls_fn)

        mu_power_share = nn.sigmoid(mu_u_source_mag)**2

        return entropy_marginal_transition, p_ratio_source, mu_u_source, A, explore_var, A_complement, mu_power_share, residual_power, Jac, y_prime_mean, s_prime_var
...
class power_coef(nn.Module):
    ...
    def __call__(self) -> jnp.ndarray:
        if self.constraint_type == "equality": # power_coef can be positive or negative
            power_coef = self.param("power_coef", init_fn=lambda key: jnp.full((), self.power_init))
        elif self.constraint_type == "inequality": # power_coef is non-negative
            inverse_softplus_power_coef = self.param("inverse_softplus_power_coef", init_fn=lambda key: jnp.full((), jnp.log(jnp.exp(self.power_init)-1.)))
...
class source(nn.Module):
    ...
    def setup(self):
        ...
        source_policy = [nn.Sequential([nn.Dense(features=h_dim), nn.relu]) for h_dim in self.h_dims_source] # [nn.Dense(features=h_dim), nn.LayerNorm(), nn.relu]
        if self.state_dependent_base_variance:
            # initialise the final layer output to 0 so that the inititial mean and log_var is 0
            source_policy.append(nn.Dense(features=self.a_dim+1, bias_init=zeros_init(), kernel_init=zeros_init()))
        else:
            # initialise the final layer output to 0 so that the inititial mean and log_var is 0
            source_policy.append(nn.Dense(features=self.a_dim, bias_init=zeros_init(), kernel_init=zeros_init()))
        self.source_policy = source_policy

        self.house_holder = self.param('house_holder', normal(), (self.a_dim,self.a_dim))
        if self.power_for_var:
            # self.decoder_axes_scales = self.param('decoder_axes_scales', constant(0.), (self.a_dim)) # softmax
            self.decoder_axes_scales = self.param('decoder_axes_scales', constant(jnp.log(jnp.exp(1.)-1.)), (self.a_dim)) # softplus
        else:
            self.decoder_axes_scales = self.param('decoder_axes_scales', constant(self.init_axes_scales), (self.a_dim)) # for elbow, worse with -3.5 or -2.5, when used with power dual or power fixed 0.05
        self.decoder_b = self.param('decoder_b', zeros_init(), (self.a_dim))

        self.flow = flow_model(h_dims_conditioner=self.h_dims_conditioner,
                               num_bijector_params=2,
                               num_coupling_layers=self.num_coupling_layers,
                               a_dim=self.a_dim)

        inverse = [nn.Sequential([nn.Dense(features=h_dim), nn.relu]) for h_dim in self.h_dims_inverse]
    ...
    def __call__(self, s, key, dynamics_params, power_coef):
        ...
        batch_log_likelihood_diagonal_Gaussian = jax.vmap(log_likelihood_diagonal_Gaussian, in_axes=(None,0,0))
        ...
        def construct_channel(epsilon=0.):

            orthogonal_matrix = associative_scan(jax.vmap(jnp.matmul), process_house_holder(self.house_holder))[-1] 
            ...
            if self.normalise_scales:
                ...
                decoder_axes_scales = jnp.sqrt(self.a_dim * decoder_axes_scales**2 / jnp.sum(decoder_axes_scales**2))
            ...
            A = orthogonal_matrix @ jnp.diag(decoder_axes_scales) # scale the columns, which scales the z-coordinates (https://www.youtube.com/watch?v=P2LTAUO1TdA)
            # A = self.house_holder
        ...
        def sample_action_and_predict_next_state(s, a_mean, a_log_var, decoder_A, decoder_b, a_cov, key, dynamics_params):
            ...
            base_dist_a = distrax.MultivariateNormalDiag(loc=a_mean, scale_diag=jnp.exp(0.5*a_log_var))
            ...
            if self.power_for_var:
                uniform_scaler = distrax.UnconstrainedAffine(matrix=jnp.sqrt(self.power_target)*jnp.eye(self.a_dim), bias=jnp.zeros(self.a_dim))
                bijector = distrax.Chain([bijector, uniform_scaler])

            dist_a = distrax.Transformed(base_dist_a, distrax.Chain([distrax.Block(distrax.Tanh(), 1), bijector]))
            squashed_a_i, ll_a = dist_a.sample_and_log_prob(seed=a_key)

            x = jnp.concatenate((s, squashed_a_i), axis=-1)
            # x = jnp.copy(squashed_a_i)
            for i, fn in enumerate(self.dynamics):
                x = fn.apply({'params': dynamics_params['params']['dynamics_' + str(i)]}, x)
                if i == 0:
            ...
            base_dist_a_inverse = distrax.MultivariateNormalDiag(loc=a_inverse_mean, scale_diag=jnp.exp(0.5*a_inverse_log_var))
            ...
            dist_a_inverse = distrax.Transformed(base_dist_a_inverse, distrax.Chain([distrax.Block(distrax.Tanh(), 1), posterior_bijector]))
            ll_a_inverse = dist_a_inverse.log_prob(jnp.clip(squashed_a_i, -1.+1e-6, 1.-1e-6))
        ...
        if self.state_dependent_base_variance:
            if self.power_for_var:
                effective_dimensionality = ((jnp.exp(get_log_var(a_scale))*decoder_axes_scales**2*self.power_target).sum(-1)**2/((jnp.exp(get_log_var(a_scale))*decoder_axes_scales**2*self.power_target)**2).sum(-1)).mean()
            else:
                effective_dimensionality = ((jnp.exp(get_log_var(a_scale))*decoder_axes_scales**2).sum(-1)**2/((jnp.exp(get_log_var(a_scale))*decoder_axes_scales**2)**2).sum(-1)).mean()
        else:
            a_cov = decoder_A @ jnp.diag(jnp.exp(a_log_var)) @ decoder_A.T
            effective_dimensionality = get_effective_dimensionality(a_cov)

        squashed_a_i, s_prime_i, ll_a_inverse, ll_a = sample_action_and_predict_next_state(s, a_mean, a_log_var, decoder_A, self.decoder_b*0., [], key, dynamics_params) # for elbow, decoder_b = 0 is (marginally) worse
        ...
        power_penalty = power_coef * (avg_power-self.power_target) # KKT stationarity condition
        ...
        if self.normalise_scales:
            ...
            loss = -mutual_information + power_penalty # + self.power_coefficient * (avg_power-self.power_target)**2 # + power_coef_loss

        return loss, ll_a_inverse, ll_a, effective_dimensionality, avg_power, decoder_axes_scales
...
class Actor(nn.Module):
    ...
    def setup(self):
        ...
        if len(self.source_mu) == 1:
            self.squash_controls_fn = lambda u: nn.sigmoid(u*5.)
        else:
            self.squash_controls_fn = lambda u: jnp.concatenate([nn.sigmoid(u[...,:63]*5.),self.unscale_action(nn.tanh(u))[...,63:]],axis=-1) # for MPL, this should be between [low,high] not [-1,1] - unscale does this

        self.flatten = Flatten()

        # self.power_target = self.power_target_per_dim * self.z_dim 

        policy = [nn.Sequential([nn.Dense(features=h_dim), nn.relu]) for h_dim in self.net_arch] # [nn.Dense(features=h_dim), nn.LayerNorm(), nn.relu]
    ...
    # @nn.compact
    def __call__(self, obs: jnp.ndarray, synergies_state, dynamics_state, key, deterministic) -> tfd.Distribution:  # type: ignore[name-defined] # mu_u_source, A, explore_var, A_complement,
        ...
        if self.base_covariance_spherical:
            ...
            log_std = jnp.repeat(jnp.clip(log_std, self.log_std_min, self.log_std_max), self.z_dim, axis=1)
        else:
            if self.cocontraction == 'state_dependent':
                a_mean, logit_std, cocontraction = jnp.split(x, [self.z_dim+self.MPL_acts, (self.z_dim+self.MPL_acts)*2], axis=-1)
            else:
        ...
        _, _, mu_u_source, A, explore_var, A_complement, mu_power_share, residual_power, Jac, _, _ = synergies_state.apply_fn(synergies_state.params, obs, dynamics_state, key, deterministic, cocontraction, self.squash_controls_fn)
        ...
        base_dist = distrax.MultivariateNormalDiag(loc=a_mean, scale_diag=nn.sigmoid(logit_std)*self.std_max)
        ...
        explore_dist = distrax.MultivariateNormalDiag(scale_diag=jnp.repeat(self.explore_coeff*jnp.sqrt(explore_var)[:,None],63-self.z_dim,axis=1))

        return base_dist, explore_dist, A, mu_u_source, A_complement, self.squash_controls_fn
...
class SACPolicy(BaseJaxPolicy):
    ...
    def build(self, key: jax.Array, lr_schedule: Schedule, qf_learning_rate: float) -> jax.Array:
        key, actor_key, qf_key, dropout_key, source_key, dynamics_key, power_key, synergies_key = jax.random.split(key, 8)
        ...
        self.dynamics = dynamics(h_dims_dynamics=[256,256],
        ...
        self.synergies = synergies(h_dims_source=[256,256],
        ...
        self.synergies_state = TrainState.create(
            ...
            params=self.synergies.init(synergies_key, obs, self.dynamics_state, synergies_key, True, jnp.zeros((obs.shape[0],1)), lambda u: u),
        ...
        self.posterior = jax.tree_util.Partial(posterior_inference, control_variables=self.control_variables)
        ...
        self.actor_state = TrainState.create(
            ...
            params=self.actor.init(actor_key, obs, self.synergies_state, self.dynamics_state, actor_key, True), # jnp.zeros((1,80)), jnp.zeros((1,80,14)), jnp.zeros((1,)), jnp.zeros((1,80,66)), 
        ...
        self.qf_state = RLTrainState.create(
            ...
            apply_fn=partial(lambda obs_variables, params, obs, action, rngs: self.qf.apply(params, obs[...,obs_variables], action, rngs=rngs), self.obs_variables),
            params=self.qf.init(
        ...
        self.ik = GradientDescentIK(self.MjModel, MjData, step_size=0.1, tol=0, alpha=0.1, jacp=jacp, jacr=jacr)
    ...
    def _predict(self, observation: np.ndarray, phase, deterministic: bool = False) -> np.ndarray:  # type: ignore[override]
        if deterministic:
            return BaseJaxPolicy.select_action(self.actor_state.params, self.actor_state, observation, self.noise_key, self.synergies_state, self.dynamics_state, phase)
        # Trick to use gSDE: repeat sampled noise by using the same noise key
        if not self.use_sde:
            self.reset_noise()
        action, _, _ = BaseJaxPolicy.sample_action(self.actor_state.params, self.actor_state, observation, self.noise_key, self.synergies_state, self.dynamics_state, phase)
        return action

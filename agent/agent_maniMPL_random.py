...
def load_policy(model_id):
    ...
    print(path)
    learned_params = pickle.load(
        open(os.path.join(path, "params_" + model_id + ".pkl"), "rb")
    )
    print("MANIPULATION agent: params loaded")

    env_id = "myoChallengeBimanual-v0"

    ordered_custom_obs_keys = [
        "myohand_qpos",
        "myohand_qvel",
        "act",
        "pros_hand_qpos",
    ...
    control_variables = [215 + i for i in range(2)] + [
        env_tmp.sim.model.joint_name2id(q) for q in q_to_control
    ]
    task_variables_keys = ["start_pos", "goal_pos"]
    task_variables = [
        idx for key in task_variables_keys for idx in list(env_tmp.key_idx[key])
    ]
    obs_variables = list(
        range(215)
    )  ## TO USE THIS NEED TO UNCOMMENT CODE IN my_policies
    ...
    def make_env(
        ...
        def _init():
            env = gym.make(
                ...
                obs_keys=ordered_custom_obs_keys,
                max_episode_steps=max_episode_steps,
            )
            if wrap:
                env = myoChallengeWrapper(
                    env,
                    control_variables,
                    obs_variables,
                    reward_id,
    ...
    model.policy.dynamics_state = model.policy.dynamics_state.replace(
        params=learned_params["dynamics"]
    )
    model.policy.actor_state = model.policy.actor_state.replace(
        params=learned_params["actor"]
    )

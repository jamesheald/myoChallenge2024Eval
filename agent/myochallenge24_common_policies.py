...
class BaseJaxPolicy(BasePolicy):
    ...
    def new_actions(self, actions, observation, touching_myo, touching_mpl, phase):
        ...
        if self.touching_mpl_count > 5 and self.touching_myo_count > 2 and self.phase_three==False:

            self.phase_three=True
            self.get_target_qpos(observation, np.array([0.1, -0.0, 0.3]))
        ...
        if np.linalg.norm(observation[0,139:143]-self.target_qpos) < 0.15 and self.phase_three and self.phase_four==False:
            ...
            self.get_target_qpos(observation, np.array([0.1, -0.0, 0.1]))
